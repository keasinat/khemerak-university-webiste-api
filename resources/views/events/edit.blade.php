...
@endpush
@section('content')
    <x-forms.patch :action="route('admin.event.update', $event)">
        <x-card>
            <x-slot name="header">{{ __('dashboard.event_edit') }}</x-slot>
                        ...
                        <div class="col-9">
                            ...
                            <div class="row">
                                ...
                                <label for="content" class="form-label">{{ __('dashboard.content') }}</label>
                                    <textarea name="content" id="content" cols="30" rows="10" class="form-control {{ $errors->has('content') ? 'is-invalid' : '' }}">{{ old('content') ?? $event->content}}</textarea>
                                </div>
                            </div>
                            <div class="row">
                                <div class="form-group col-12">
                                    <label for="description">{{ __('dashboard.description') }}</label>
                                        <textarea name="description" id="description" cols="30" rows="5"  class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}">{{old('description') ?? $event->description}}</textarea>
                            ...
                            <div class="row">
                                ...
                                <input type="text" name="location" class="form-control {{ $errors->has('location') ? 'is-invalid' : '' }}" value="{{ old('location') ?? $event->location }}">
                        ...
                        <div class="col-3">
                            ...
                            <div class="form-group col-12">
                                ...
                                <div class="input-group">
                                    ...
                                    <input id="thumbnail" class="form-control {{ $errors->has('thumbnail') ? 'is-invalid' : '' }}" type="text" name="thumbnail" value="{{ old('thumbnail') ?? $event->thumbnail}}">
                            ...
                            <div class="form-group col-12">
                                ...
                                <select name="is_published" id="is_published" class="form-control">
                                        @foreach (pulishedOpt() as $k => $item)
                                            <option value="{{ $k }}" {{ $event->is_published == $k ? 'selected' : '' }}>{{ $item }}</option>
                                        @endforeach
                                    </select>
                            </div>
