...
@endpush
@section('content')
    <x-forms.patch :action="route('admin.menu.subject.update', $subject)">
        <x-card>
            <x-slot name="header">{{ __('Edit Subject') }}</x-slot>
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="title_km" required
                                ...
                                value="{{ old('title_km') ?? $subject->title_km }}">
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="title_en"
                                ...
                                value="{{ old('title_en') ?? $subject->title_en }}">
                                ...
                                @if (isset($academics))
                                    @foreach ($academics as $category)
                                        ...
                                        <option value="{{ $category->id }}"
                                            {{ (old('academic_id') !== null ) ? ( old('academic_id') == $category->id ? 'selected':'') : ($subject->academic_id == $category->id ? 'selected':'') }}
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input id="thumbnail"
                                    ...
                                    name="thumbnail" value="{{ old('thumbnail') ?? $subject->thumbnail }}">
                        ...
                        <div class="form-group">
                            ...
                            <select name="is_published" id="is_published" class="form-control">
                                @foreach (pulishedOpt() as $k => $item)
                                    <option value="{{ $k }}" {{ (old('is_published') !== null) ? old('is_published') == $k ? 'selected' : '' : ($k == $subject->is_published ? 'selected' :'') }}>{{ $item }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <textarea name="highlight_km" id="highlight_km" cols="30" rows="6"
                                class="form-control {{ $errors->has('highlight_km') ? 'is-invalid' : '' }}">{{ old('highlight_km') ?? $subject->highlight_km }}</textarea>
                    ...
                    <div class="col-sm-6">
                        <div class="form-group">
                            ...
                            <textarea name="highlight_en" id="highlight_en" cols="30" rows="6"
                                class="form-control {{ $errors->has('highlight_en') ? 'is-invalid' : '' }}">{{ old('highlight_en') ?? $subject->highlight_en }}</textarea>
                    ...
                    <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <textarea name="description_km" id="description_km" cols="50" rows="5"
                                class="form-control {{ $errors->has('description_km') ? 'is-invalid' : '' }}">{!! old('description_km') ?? $subject->description_km !!}</textarea>
                    ...
                    <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <textarea name="description_en" id="description_en" cols="50" rows="5"
                                class="form-control {{ $errors->has('description_en') ? 'is-invalid' : '' }}">{!! old('description_en') ?? $subject->description_en !!}</textarea>
