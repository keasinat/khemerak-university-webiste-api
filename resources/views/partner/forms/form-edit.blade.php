    ...
    <div class="row">
        <div class="col-sm-6">
            ...
            <div class="form-group">
                ...
                <input type="text" name="link" id="" class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" value="{{ old('link') ?? $partner->link }}">
                @if($errors->has('link'))
                    <div class="invalid-feedback">
                        {{ $errors->first('link') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <div class="input-group">
                    ...
                    <input id="logo" class="form-control {{ $errors->has('logo') ? 'is-invalid' : '' }}" type="text" name="logo" value="{{ old('logo') ?? $partner->logo }}">
                    @if($errors->has('logo'))
                        <div class="invalid-feedback">
                            {{ $errors->first('logo') }}
                        </div>
            ...
            <div class="form-group">
                ...
                <select name="is_published" id="is_published" class="form-control">
                    @foreach (pulishedOpt() as $k => $item)
                        <option value="{{ $k }}" {{ $partner->is_published == $k ? 'selected' : ''}}>{{ $item }}</option>
                    @endforeach
                </select>
            </div>
        </div>
